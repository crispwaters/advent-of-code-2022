...
const limit = 30
const dfs = (cur, openValvesOrder, turn) => {
  ...
  for (const next of toOpen) {
    ...
    const tPressure = Math.max((limit - turn - pathLength + 1) * valves[next].rate, 0) + Math.max(...dfs(next, `${[...openValvesOrder.split(','), next].join(',')}`, turn + pathLength))
    results.push(tPressure)
  }
  return results
}
console.log(Math.max(...dfs('AA', '', 1)))