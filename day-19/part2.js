...
class Blueprint {
  ...
  calculateMaxGeodCount(nMinutes = 32) {
    const startState = {
      robots: {
        ore: 1,
        clay: 0,
    ...
    for (let minute = 1, remaining = nMinutes; minute <= nMinutes; minute++, remaining--) {
      ...
      for (const key of Object.keys(states)) {
        ...
        if (minute === nMinutes) {
          ...
          for (const material of Blueprint.MATERIALS) {
            ...
            if ((state.robots[material] * remaining) + state.materials[material] >= (remaining * this.maxCosts[material])) continue
...
const blueprints = readBlueprints(input).slice(0, 3)

for (const blueprint of blueprints) {
  blueprint.calculateMaxGeodCount()
}
console.log(blueprints.reduce((acc, cur) => acc * cur.maxGeodeCount, 1))